...
export const getMeIntoDB = async (userId: string, role: string) => {
  let result = null;
  if (role === 'student') {
...
export const changeStatusIntoDB = async (
  id: string,
  payload: { status: string },
) => {
  const isUserExist = await User.findByIdAndUpdate(id, payload, {
    new: true,
  });
